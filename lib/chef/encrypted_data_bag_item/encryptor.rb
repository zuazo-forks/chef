  ...
  module Encryptor
    ...
    class Version3Encryptor < Version2Encryptor
      ...
      def initialize(plaintext_data, key, iv=nil)
        ...
        key_length = Chef::EncryptedDataBagItem.key_length
        if key.length < key_length
          Chef::Log.warn("Data Bag secret too short, #{key.length} bytes. Should be at least #{key_length} bytes length.")
        end

        # split the hmac key form the key
        keys = key.unpack("A#{Chef::EncryptedDataBagItem.hmac_key_length}Z*")
        @hmac_key = keys[0]
        @key = keys[1]
      end
      ...
      def hmac
        @hmac ||= begin
          ...
          raw_hmac = OpenSSL::HMAC.digest(digest, hmac_key, data_to_hmac)
          Base64.encode64(raw_hmac)
        end
      end
    end
